document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById(
    "alunoNome"
  ).textContent = `Tarefas Registradas - ${alunoNome}`;

  const tabela = document
    .getElementById("tabelaTarefas")
    .querySelector("tbody");
  const btnNovaAtividade = document.getElementById("btnNovaAtividade");
  const formAtividadeContainer = document.getElementById(
    "formAtividadeContainer"
  );
  ...
  async function carregarTarefas() {
    ...
    try {
      ...
      tabela.innerHTML =
        '<tr><td colspan="5">Erro ao carregar tarefas.</td></tr>';
    }
  }
  ...
  btnNovaAtividade.addEventListener("click", function () {
    ...
    document.getElementById("formAtividadeTitulo").textContent =
  ...
  // ROTINAS
  const tabelaRotinas = document
    .getElementById("tabelaRotinas")
    .querySelector("tbody");
  ...
  function carregarRotinas() {
    ...
    fetch(`/api/rotinas/aluno/${alunoId}`)
      ...
      .then((rotinas) => {
        if (!rotinas || rotinas.length === 0) {
          tabelaRotinas.innerHTML =
            '<tr><td colspan="3">Nenhuma rotina registrada.</td></tr>';
        } else {
          tabelaRotinas.innerHTML = "";
      ...
      .catch(() => {
        tabelaRotinas.innerHTML =
          '<tr><td colspan="3">Erro ao buscar rotinas.</td></tr>';
      });
  }
  carregarRotinas();
  ...
  btnNovaRotina.addEventListener("click", function () {
    ...
    document.getElementById("formRotinaTitulo").textContent =
  ...
  tabelaRotinas.addEventListener("click", async function (e) {
    if (e.target.classList.contains("btn-editar-rotina")) {
      ...
      document.getElementById(
        "formHorarioTitulo"
      ).textContent = `Ajustar Horário da Rotina: ${nome}`;
            ...
            dias.forEach((dia) => {
              ...
              if (disp) {
                if (document.querySelector(`[name=horaInicio-${dia.nome}]`)) {
                  // Formatar para HH:MM
                  const horaInicio = disp.horaInicio
                    ? disp.horaInicio.slice(0, 5)
                    : "";
                  const horaFim = disp.horaFim ? disp.horaFim.slice(0, 5) : "";
                  document.querySelector(
                    `[name=horaInicio-${dia.nome}]`
                  ).value = horaInicio;
                  document.querySelector(`[name=horaFim-${dia.nome}]`).value =
                    horaFim;
                  document
                    .querySelector(`[name=horaInicio-${dia.nome}]`)
                    .setAttribute("data-disp-id", disp.id);
                }
              } else {
                if (document.querySelector(`[name=horaInicio-${dia.nome}]`)) {
                  document
                    .querySelector(`[name=horaInicio-${dia.nome}]`)
                    .removeAttribute("data-disp-id");
                }
              }
            });
          }
      ...
      for (const rd of rotinaDias) {
        const disp = await fetch(`/api/buscar/disponibilidade/${rd.id}`).then(
      ...
      for (const atv of atividades) {
        ...
        for (const [rotinaDiaId, info] of Object.entries(tempoDisponivel)) {
          if (info.minutos >= atv.duracao) {
            ...
            const hora = Math.floor(info.proximoInicio / 60)
              .toString()
              .padStart(2, "0");
      ...
      for (const dia of diasSemana) {
        ...
        if (rotinaDia) {
          ...
          const atividades = await fetch(
            `/api/rotinaatividades/rotinadia/${rotinaDia.id}`
          )
            .then((res) => res.json())
            .then((arr) => (Array.isArray(arr) ? arr : []));
          if (atividades.length > 0) {
            atividades.forEach((ra) => {
              html += `<div class=\"atividade-alocada\" data-id=\"${
                ra.atividadeId || ra.id
              }\" style=\"margin:8px 0; padding:6px; background:#fff; border-radius:6px; box-shadow:0 1px 2px #0001; cursor:pointer;\">
                                <span style=\"font-weight:500;\">${
                                  ra.nome || ra.Atividade?.nome || "Atividade"
                                }</span><br/>
                                <span style=\"font-size:0.95em; color:#555;\">Início: ${
                                  ra.horaInicio
                                    ? ra.horaInicio.slice(0, 5)
                                    : "--:--"
                                }</span>
                            </div>`;
            });
            ...
            html +=
              '<span style="color:#888; font-size:0.95em;">Nenhuma atividade alocada</span>';
          }
        } else {
          html +=
            '<span style="color:#888; font-size:0.95em;">Nenhuma rotina para este dia</span>';
        }
        html += "</div>";
      }
      html += "</div>";
      ...
      quadro.querySelectorAll(".atividade-alocada").forEach((div) => {
        div.onclick = async function () {
          ...
          document.getElementById("modalDetalheAtividade").style.display =
            "flex";
          // Buscar detalhes da atividade
          const atividade = await fetch(
            `/api/buscar/atividade/${atividadeId}`
          ).then((res) => res.json());
  ...
  document
    .getElementById("cancelarHorario")
  ...
  document
    ...
    .addEventListener("submit", async function (e) {
      ...
      const horarios = dias.map((dia) => ({
        ...
        horaInicio: document.querySelector(`[name=horaInicio-${dia.nome}]`)
          .value,
        horaFim: document.querySelector(`[name=horaFim-${dia.nome}]`).value,
        dispId: document
          .querySelector(`[name=horaInicio-${dia.nome}]`)
          .getAttribute("data-disp-id"),
      }));
      ...
      async function getOrCreateRotinaDia(rotinaId, diaSemana, diaSemanaInt) {
        let rotinaDia = await fetch(`/api/rotinadias/rotina/${rotinaId}`)
          .then((res) => res.json())
          .then((rotinasDias) =>
            (rotinasDias || []).find((rd) => rd.diaSemana == diaSemanaInt)
          );
      ...
      try {
        for (const h of horarios) {
          if (h.horaInicio && h.horaFim) {
            const rotinaDiaId = await getOrCreateRotinaDia(
              rotinaId,
              h.diaSemana,
              h.diaSemanaInt
            );
